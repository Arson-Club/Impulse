...
with setting up Impulse.

Additionally, this guide will assume you have [configured player forwarding](https://docs.papermc.io/velocity/player-information-forwarding) and are running your proxy in online mode.
...
servers must have docker installed. You can follow the [official docker installation guide](https://docs.docker.com/get-docker/) to install docker on your machine.

In this example, we will running our backend servers on the same machine as our proxy. If you run your proxy on a
seperate machine, docker is not required for that computer.

> [!TIP]
> For more information on the docker broker itself, see the [docker broker documentation](brokers/docker.md). You can
> also find more complex examples in our [Examples](examples.md) section.
